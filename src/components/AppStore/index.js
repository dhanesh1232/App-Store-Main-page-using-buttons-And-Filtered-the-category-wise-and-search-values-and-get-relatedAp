...
class AppStore extends Component {
  state = {isActiveTab: appsList[0].category, searchInput: ''}

  getFilteredAppsList = searchResult => {
  ...
  render() {
    ...
    const filteredApps = this.getFilteredAppsList(searchResult)
        ...
        <ul className="list-items">
          {tabsList.map(each => (
            <TabItem
              tabItemList={each}
              key={each.tabId}
              isActive={each.tabId === isActiveTab}
              isTabSectionChange={this.changeActiveTabId}
            />
          ))}
        </ul>
